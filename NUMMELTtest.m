...
res = 20;
y = linspace(0, 2*pi, res);
x = ones(1,res)*2*pi - y;

sinx = sin(x);
cosy = cos(y);

maxNUMMELT = zeros(1,res);
...
for i = 1:res
    ...
    MU                  =   [1; 1];  % Viscosity max/min
    MU_inc              =       0;     % Amount viscosity increases on melt
    H_activ             = [375; 320];  % Changes temperature dependence of viscosity max/min
    ...
    depth               =     2900;
    time_final          =     1e-1;  % Total simulation time
    ...
    % MECHANICAL MESH
    ney                 =    30; % Number of elements in vertical direction
    ...
    % adjust thickness to line up with thermal mesh
    res_t = L(2)*2/ney_t;
    thickness = floor(thickness_exact/res_t)*res_t;
    ...
    MARK.MU             =    MU(2)*ones(1,nm); % default mantle value
    ...
    MARK.MU(prev_melt_bool)     =    MARK.MU(prev_melt_bool) + MU_inc;
    MARK.H_activ(prev_melt_bool)=    MARK.H_activ(prev_melt_bool) + H_activ_inc;
    MARK.NUMMELT(prev_melt_bool)=    MARK.NUMMELT(prev_melt_bool) + 1;
    ...
    Mweights            =    accumarray(MARK2ELEM_t',ones(size(MARK2ELEM_t)),[nel_t 1]);
    PHASEsum            =    accumarray(MARK2ELEM_t',MARK.PHASES,[nel_t 1]);
    PHASES_t            =    round(PHASEsum./Mweights);
    NUMMELTsum          =    accumarray(MARK2ELEM_t',MARK.NUMMELT,[nel_t 1]);
    NUMMELT_t           =    round(NUMMELTsum./Mweights);
    TIMEMELTsum         =    accumarray(MARK2ELEM_t',MARK.TIMEMELT,[nel_t 1]);
    TIMEMELT_t          =    round(TIMEMELTsum./Mweights);
    ...
    MU_perturb                  =    arrayfun(@thermal_profile_perturb, TEMP_nodes, H_activ_nodes);
    ...
    %--------------------------------------------------------------------------
    Vx                 =    ones(1, nnod)*sinx(i);
    Vy                 =    ones(1, nnod)*cosy(i);
    ...
    while folder_exists
        run_title = strjoin(["Bu_", string(Bu), "_Mu_", ...
            string(MU(1)), "_", string(MU(2)), "_H_activ_", string(H_activ(1)), ...
            "_", string(H_activ(2)), "_asp_", string(asp), "_ney_", string(ney), ...
            "_", string(num_sim)], '');
    ...
    save(static_file_name, "Ra", "Bu", "MU", "RHO", "RHO_inc", "H_activ", "H_activ_inc", ...
        "thickness", "time_final", "dt_save", "asp", "ney", "L")
    ...
    while time<time_final
       ...
       %----------------------------------------------------------------------
        ...
        if min(Mweights_t)<1
            ...
            MARK                =    reseed2_activ(MARK, asp, L, ney_t, nmy, ELEM2NODE_t, RHO_nodes, MUPHASE_nodes, H_activ_nodes, PHASES_t, NUMMELT_t, TIMEMELT_t);
        ...
        Mweights            =    accumarray(MARK2ELEM_t',ones(size(MARK2ELEM_t)),[nel_t 1]); % counts how many markers in each mechanical elemet
        PHASEsum            =    accumarray(MARK2ELEM_t',MARK.PHASES,[nel_t 1]);
        PHASES_t            =    round(PHASEsum./Mweights);
        NUMMELTsum          =    accumarray(MARK2ELEM_t',MARK.NUMMELT,[nel_t 1]);
        NUMMELT_t           =    round(NUMMELTsum./Mweights);
        ...
        if time_save > dt_save
            ...
            save(file_name, "MARK", "TEMP_nodes", "RHO_nodes", "MU_nodes", "H_activ_nodes", "new_melt", "Vx", "Vy", "time", "maxNUMMELT")
            time_save   =    0;
        end
    
        maxNUMMELT(i) = max(MARK.NUMMELT(:));
    ...
    clear GCOORD_char IND_char U_char V_char SHP_char Vx_mark Vy_mark MU_perturb MU_nodes
