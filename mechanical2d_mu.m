function [Vel, Pressure] = mechanical2d_mu(ELEM2NODE, PHASES, GCOORD, MU_el, Temp, Ra, G, Bc_ind, Bc_val, nip, elemType, reorder, method)
...
switch method
    ...
    case 'std'
        ...
        for iel = 1:nel
            ...
            for ip=1:nip
                ...
                Rhs_elem(1,:)= Rhs_elem(1,:) + weight*ERho_ip*G(1)*Ni';
                Rhs_elem(2,:)= Rhs_elem(2,:) + weight*ERho_ip*G(2)*Ni';
            end
            %==============================================================
    ...
    case 'opt'
        ...
        for ib = 1:nblo
            ...
            ED       = MU_el(il:iu);
            ERho     = reshape( Temp(ELEM2NODE(:,il:iu)), nnodel, nelblo);
            ...
            for ip=1:nip
                ...
                % -----------------------Rhs vector------------------------
                Rhs_block(:,1:2:nedof) = Rhs_block(:,1:2:nedof) + G(1)*(ERho_ip'.*weight)*Ni';
                Rhs_block(:,2:2:nedof) = Rhs_block(:,2:2:nedof) + G(2)*(ERho_ip'.*weight)*Ni';
...
if exist(['cs_transpose.' mexext], 'file') == 3
    ...
    Rhs         = Ra*Rhs - TMP*Bc_val;
else
    Rhs         = Ra*Rhs -  A*Vel - A'*Vel;
end
A           = A(Free,Free);
...
while (div_max>div_max_uz  && uz_iter<uz_iter_max)
    ...
    if exist(['cs_ltsolve.' mexext], 'file') == 3 && exist(['cs_lsolve.' mexext], 'file') == 3
        ...
        Vel(Free(perm)) = L'\(L\(Rhs(Free(perm))));
    end
