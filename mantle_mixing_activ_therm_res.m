...
FUG                  =   [0; 1];  % Viscosity max/min
FUG_inc              =        0;     % Amount viscosity increases on melt
...
PHASEsum_m            =    accumarray(MARK2ELEM',MARK.PHASES,[nel 1]);
PHASES              =    round(PHASEsum_m./Mweights);

Mweights_t            =    accumarray(MARK2ELEM_t',ones(size(MARK2ELEM_t)),[nel_t 1]);
PHASEsum            =    accumarray(MARK2ELEM_t',MARK.PHASES,[nel_t 1]);
PHASES_t            =    round(PHASEsum./Mweights_t);
NUMMELTsum          =    accumarray(MARK2ELEM_t',MARK.NUMMELT,[nel_t 1]);
NUMMELT_t           =    round(NUMMELTsum./Mweights_t);
TIMEMELTsum         =    accumarray(MARK2ELEM_t',MARK.TIMEMELT,[nel_t 1]);
TIMEMELT_t          =    round(TIMEMELTsum./Mweights_t);
...
%MU_perturb                  =    arrayfun(@thermal_profile_perturb_activ_therm_res, ...
 %                                         TEMP_nodes, H_activ_nodes, FUGPHASE_nodes);
MU_perturb                  =    arrayfun(@thermal_profile_perturb_activ, TEMP_nodes, H_activ_nodes);
...
while folder_exists
    run_title = strjoin(["Bu_", string(Bu), "_Mu_", ...
        string(FUG(1)), "_", string(FUG(2)), "_H_activ_", string(H_activ(1)), ...
...
save(static_file_name, "Ra", "Bu", "FUG", "FUG_inc", "RHO", "RHO_inc", "H_activ", ...
    "H_activ_inc", "thickness_exact", "time_final", "dt_save", "asp", "ney", "L")
...
while time<time_final
   ...
   %----------------------------------------------------------------------
    ...
    Mweights_t            =    accumarray(MARK2ELEM_t',ones(size(MARK2ELEM_t)),[nel_t 1]); % counts how many markers in each mechanical elemet
    PHASEsum            =    accumarray(MARK2ELEM_t',MARK.PHASES,[nel_t 1]);
    PHASES_t            =    round(PHASEsum./Mweights_t);
    NUMMELTsum          =    accumarray(MARK2ELEM_t',MARK.NUMMELT,[nel_t 1]);
    NUMMELT_t           =    round(NUMMELTsum./Mweights_t);
    ...
    if time_save > dt_save
        ...
        save(file_name, "MARK", "TEMP_nodes", "RHO_nodes", "MU_nodes", ...
            "H_activ_nodes", "new_melt", "Vx", "Vy", "time")
        time_save   =    0;
    end
